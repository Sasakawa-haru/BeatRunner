#include "GameCsvReader.h"
#include <fstream>
#include <assert.h>

GameCsvReader::GameCsvReader(std::string filename)
{
	all.clear();
...
int GameCsvReader::GetLines()
{
	return all.size();
}

int GameCsvReader::GetColumns(int line)
{
	assert(line < GetLines());
	return all[line].record.size();
}

std::string GameCsvReader::GetString(int line, int column)
{
	assert(line < GetLines());
...
int GameCsvReader::GetInt(int line, int column)
{
...
float GameCsvReader::GetFloat(int line, int column)
{
	std::string str = GetString(line, column);
	if (str == "") {
		return 0.0f;
